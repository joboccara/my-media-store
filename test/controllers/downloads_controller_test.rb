...
class DownloadsControllerTest < TestHelperTraining
  test 'a downloaded product appears in the library' do
    ...
    post downloads_url, params: { user_id: user.id, product_id: book_to_download_1.id }
    post downloads_url, params: { user_id: user.id, product_id: book_to_download_2.id }
  ...
  test 'a user can download an product only once' do
    user = User.create!(first_name: 'Bob')
    book_to_download = create_book(title: 'Radical Candor', content: 'Contents of Book')

    post downloads_url, params: { user_id: user.id, product_id: book_to_download.id }
